...
def _expand_dataset_dirs(dataset_dirs: list[str]) -> list[str]:
    """Expand dataset dirs the same way as BaseLerobotDataset._expand_dataset_dir.
    Priority for each entry:
      1. Has meta/info.json -> single valid dataset
      2. Has dataloader.json -> load paths from it
      3. Scan subdirectories with meta/info.json
    """
    expanded = []
    for ds_dir in dataset_dirs:
        ds_path = Path(ds_dir)
        if not ds_path.exists():
            logger.warning("Dataset dir does not exist, skipping: %s", ds_dir)
            continue
        # Priority 1: single valid dataset
        if (ds_path / "meta" / "info.json").exists():
            expanded.append(ds_dir)
            continue
        # Priority 2: dataloader.json
        ...
        if json_config.exists():
            ...
            for p in raw_paths:
                ...
                if path.exists() and (path / "meta" / "info.json").exists():
                    ...
                    logger.warning("Path from dataloader.json not valid, skipping: %s", path)
            if resolved:
                logger.info("Loaded %d dataset(s) from '%s'", len(resolved), json_config)
                expanded.extend(sorted(resolved))
                continue
        # Priority 3: scan subdirectories
        subdatasets = sorted(
            str(sub) for sub in ds_path.iterdir()
            if sub.is_dir() and (sub / "meta" / "info.json").exists()
        )
        if subdatasets:
            logger.info("Expanded '%s' into %d subdatasets", ds_dir, len(subdatasets))
            expanded.extend(subdatasets)
        else:
            logger.warning("No valid datasets found in '%s', skipping.", ds_dir)
    return expanded


def _get_task_variants(task_raw: str) -> list[str]:
    """镜像 FaSTWaMProcessor.augment_instruction() 的文本变换，返回所有可能的指令变体。

    训练时 augment_instruction() 会对原始 task 文本做两类变换：
      1. @ 分隔符（Galaxea 双语格式）："中文指令@English instruction"
         → 运行时按 use_zh_instruction 取一侧。
         → precompute 两侧都缓存，保证无论 use_zh_instruction 取哪个值，hash 都命中。
      2. 高层指令拼接（需要 coarse_task）：
         "[High]: coarse, [Low]: low"  vs  仅保留 low
         → 此处 coarse_task 来自 parquet 运行时，precompute 阶段不可知；
           但 drop_high_level_prob 默认 1.0（永远只用 low），故此分支不影响 hash。
           若某 task 有 coarse 变体，调用方传入 coarse_task 参数。

    Args:
        task_raw: tasks.parquet (v3.0) / tasks.jsonl (v2.1) 里读到的原始 task 字符串

    Returns:
        去重后的所有可能 task 文本列表（不含 DEFAULT_PROMPT 外壳）
    """
    ...
    if "@" in task_raw:
        # Galaxea 双语格式："中文部分@English part"
        parts = task_raw.split("@", 1)
...
def _read_unique_prompts(dataset_dirs: list[str]) -> list[str]:
    # Expand root dirs into individual dataset dirs
    dataset_dirs = _expand_dataset_dirs(dataset_dirs)
    ...
    for ds_dir in dataset_dirs:
        ds_path = Path(ds_dir)
        tasks_parquet_path = ds_path / "meta" / "tasks.parquet"
        tasks_path = ds_path / "meta" / "tasks.jsonl"

        if tasks_parquet_path.exists():
            # v3.0 format: parquet with task-text as row index and task_index column
            import pyarrow.parquet as pq
            table = pq.read_table(tasks_parquet_path)
            df = table.to_pandas()
            # The task text is stored as the row index (__index_level_0__)
            if df.index.name is None and "__index_level_0__" in df.columns:
                df = df.set_index("__index_level_0__")
            for task_text in df.index:
                raw = str(task_text).strip()
                if not raw:
                    continue
                total_task_rows += 1
                for task_var in _get_task_variants(raw):
                    prompt = DEFAULT_PROMPT.format(task=task_var)
                    if prompt not in seen:
                        seen.add(prompt)
                        prompts.append(prompt)
        elif tasks_path.exists():
            # v2.1 format: [{"task_index": 0, "task": "..."}]
            with tasks_path.open("r", encoding="utf-8") as f:
                for line_idx, line in enumerate(f, start=1):
                    ...
                    total_task_rows += 1
                    for task_var in _get_task_variants(str(record["task"])):
                        prompt = DEFAULT_PROMPT.format(task=task_var)
                        if prompt not in seen:
                            seen.add(prompt)
                            prompts.append(prompt)
            ...
            raise FileNotFoundError(
                f"Missing tasks file in '{ds_dir}': "
                f"expected meta/tasks.parquet (v3.0) or meta/tasks.jsonl (v2.1)"
            )
        ...
        with torch.no_grad():
            for start in range(0, len(prompts), DEFAULT_BATCH_SIZE):
                ...
                for i, prompt in enumerate(batch_prompts):
                    hashed = hashlib.sha256(prompt.encode("utf-8")).hexdigest()
                    # ── DEBUG: 打印每条 prompt 和对应 hash ────────────────
                    print(f"[DEBUG precompute] prompt = {repr(prompt)}")
                    print(f"[DEBUG precompute] hash   = {hashed}")
                    # ─────────────────────────────────────────────────────
