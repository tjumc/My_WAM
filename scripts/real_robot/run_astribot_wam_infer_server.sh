...
if [[ -z "${CHECKPOINT}" ]]; then
  echo "ERROR: please set CHECKPOINT=/path/to/wam_checkpoint.pt" >&2
  exit 1
fi

if [[ -z "${DATASET_STATS}" ]]; then
  echo "ERROR: please set DATASET_STATS=/path/to/stats.json" >&2
  exit 1
fi
...
python experiments/astribot/fastwam_portal_server.py \
  --config "${CONFIG}" \
