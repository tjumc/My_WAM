...
sys.path.append(str(robowin_root) + "/description/utils")
...
class ClientModel:
    ...
    def step(self, obs):
        # print("enter policy step function")
        # print(list(obs.keys()))
        ...
        left_qpos = np.expand_dims(np.array(obs['joint_action']['left_arm']), axis=0)     # shape (T, 7)
        right_qpos = np.expand_dims(np.array(obs['joint_action']['right_arm']), axis=0)    # shape (T, 7)        
        ...
        left_grip = np.expand_dims(np.array(obs["joint_action"]["left_gripper"]), axis=0)    # shape (T,)
        right_grip = np.expand_dims(np.array(obs["joint_action"]["right_gripper"]), axis=0)  # shape (T,)
        left_grip = np.expand_dims(left_grip, axis=0)    # shape (T,)
        right_grip = np.expand_dims(right_grip, axis=0)
        # left_grip = 1 - left_grip 
        # right_grip = 1 - right_grip
        ...
        #     right_grip[:, None]             # (T,1)
        # ], axis=-1)
        abs_qpos = np.concatenate([left_qpos,left_grip,right_qpos,right_grip],axis=-1)
        # print("enter vision_record")
        self.vision_record.append(image_obs)
        query = {
                "domain_id": 6,
                #"proprio": json_numpy.dumps(abs_eef.squeeze(0)), # (1, 14)
                "proprio": json_numpy.dumps(abs_qpos.squeeze(0)), # (1, 14)
        ...
        action = json_numpy.loads(response.json()['action'])
        return action
...
def _rollout(env, policy):
    ...
    for j in range(int(env.step_lim/policy.chunk_size)):
        
        # print(f"generate action in policy step")
        ...
        #left_gripper = actions[:, 6:7]  # (B, 1)  # Use 9:10 to keep 2D shape
        ...
        #left_qpos = actions[:,:6]
        #left_new = np.concatenate([left_qpos,left_grip],axis=1)
        ...
        #right_gripper = actions[:, 12:13]
        #right_grip = 1 - 2 * (right_gripper > 0.7)
        #right_qpos = actions[:,7:12]
        #right_new = np.concatenate([right_qpos,right_grip],axis=1)
        #rollout_action = np.concatenate([left_new, right_new], axis=1)  # (B, 16)
        for action in tqdm(actions[:policy.chunk_size]):
            #env.take_action(action, action_type='ee')  # target_pose: np.array([x, y, z, qx, qy, qz, qw])
            env.take_action(action, action_type='qpos')  # target_pose: np.array([q1,q2,q3,q4,q5,q6,grip])
            obs = env.get_obs()
            #obs['endpose']['left_endpose'] = list(action[:7].reshape(7,))
            #obs['endpose']['right_endpose'] = list(action[8:-1].reshape(7,))
            # obs['joint_action']['left_arm'] = list(action[:7].reshape(7,))
            # obs['joint_action']['right_arm'] = list(action[8:-1].reshape(7,))
...
def eval_episodes(task_name, task_config, policy, test_num=10, seed=0, eval_log_dir=None, instruction_type=None):
	...
	#import ipdb;ipdb.set_trace()
        ...
        #instruction = args["task_name"] # use task name as instruction
    ...
    t2 = time.time()
    print(f"task time cost {t2-t1}s")
