...
class VideoDecoderCache:
    """Thread-safe LRU cache for video decoders to avoid memory leaks."""
    ...
    def get_decoder(self, video_path: str):
        ...
        with self._lock:
            ...
            # 如果超出容量，弹出最旧的解码器并清理内存
            if len(self._cache) >= self.max_size:
                oldest_path, (old_decoder, old_file_handle) = self._cache.popitem(last=False)
                if hasattr(old_file_handle, 'close'):
                    old_file_handle.close()
                del old_decoder # 提示系统回收C++对象
...
# class VideoDecoderCache:
#     """Thread-safe cache for video decoders to avoid expensive re-initialization.
#     适配老版本 torchcodec 0.2.1（仅支持字符串路径，不支持fsspec文件句柄）
#     """
...
#         # 强制转为字符串路径（老版本torchcodec唯一支持的格式）
#         video_path = str(video_path)

#         with self._lock:
#             if video_path not in self._cache:
#                 # ✅ 核心修复：直接传入路径字符串，不使用任何文件句柄
