...
# ── Compatibility patch ──────────────────────────────────────────────────────
# Older datasets versions used feature type name 'List'; newer versions renamed
# it to 'Sequence'. Parquet files written with the old version embed 'List' in
# their Arrow schema metadata, causing generate_from_dict() to raise ValueError.
# Register 'List' as an alias so the new datasets library can still read them.
...
#STATS_PATH = "meta/stats_midea.json"
...
def align_dataset_columns(dataset: Dataset, target_features: Features | dict) -> Dataset:
    """仅校验字段名，删除Dataset中与target_features不匹配的字段"""
    ds_cols = dataset.column_names
    tgt_cols = list(target_features.keys()) if isinstance(target_features, Features) else list(target_features)
    keep_cols = [col for col in ds_cols if col in tgt_cols]
    return dataset.select_columns(keep_cols)
...
def load_nested_dataset(
    ...
    if episodes is None:
        ...
        if features:
            dataset = Dataset.from_parquet([str(p) for p in paths])
            dataset = align_dataset_columns(dataset, features).cast(features)
        
            return dataset

        else:
            return Dataset.from_parquet([str(p) for p in paths], features=features)
    ...
    if features is not None:
        # PyArrow table.cast() requires field order to match exactly.
        # Parquet files from different pipelines/versions may store columns in
        # a different order than features.arrow_schema — reorder first.
        target_names = [field.name for field in features.arrow_schema]
        available = set(table.schema.names)
        select_names = [n for n in target_names if n in available]
        table = table.select(select_names)
...
def write_my_stats(stats: dict, save_dir: Path) -> None:
    """Serialize and write dataset statistics to their standard file path.
    ...
    """
    serialized_stats = serialize_dict(stats)
    write_json(serialized_stats, save_dir)
...
def load_tasks(local_dir: Path) -> pandas.DataFrame:
    """Load tasks as a DataFrame with task-description text as index and a `task_index` column.

    Priority:
      1. meta/tasks.parquet  — v3.0 format: index=task_text, column task_index=int (always present in v3.0)
      2. meta/tasks.jsonl    — v2.1 format: {"task_index": int, "task": str}

    The returned DataFrame always looks like:
        index (task text)        task_index
        "folding paper box."     0
        "Fold napkin."           1
    so that  df.iloc[task_idx].name  returns the human-readable task description.
    """
    parquet_path = local_dir / DEFAULT_TASKS_PATH
    jsonl_path = local_dir / LEGACY_TASKS_PATH

    # ── Priority 1: tasks.parquet (v3.0, always present) ────────────────────
    if parquet_path.exists():
        tasks = pd.read_parquet(parquet_path)
        # Ensure the index holds task text (not a numeric RangeIndex).
        # The parquet stores task text as the row index (__index_level_0__).
        if tasks.index.name is None and "__index_level_0__" in tasks.columns:
            tasks = tasks.set_index("__index_level_0__")
        return tasks

    # ── Priority 2: tasks.jsonl (v2.1) ──────────────────────────────────────
    if jsonl_path.exists():
        ...
        if records:
            ...
            task_texts = [r["task"] for r in records]
            task_indices = [r["task_index"] for r in records]
            return pd.DataFrame({"task_index": task_indices}, index=task_texts)

    raise FileNotFoundError(
        f"No tasks file found. Expected '{parquet_path}' (v3.0) or '{jsonl_path}' (v2.1)."
    )
