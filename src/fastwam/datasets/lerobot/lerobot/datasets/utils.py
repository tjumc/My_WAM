...
EPISODES_V3_PATH = "meta/episodes"  # v3.0: directory with parquet files
STATS_PATH = "meta/stats.json"
EPISODES_STATS_PATH = "meta/episodes_stats.jsonl"
TASKS_PATH = "meta/tasks.jsonl"
TASKS_PARQUET_PATH = "meta/tasks.parquet"  # v3.0: primary task file
SUB_TASKS_PATH = "meta/sub_tasks.jsonl"  # v3.0: alternative task file (deprecated)
...
def load_tasks(local_dir: Path) -> tuple[dict, dict]:
    """Load tasks from tasks.parquet (v3.0, primary) or tasks.jsonl (v2.1, fallback).

    Priority:
      1. meta/tasks.parquet — v3.0 format: index=task_text, column task_index=int
      2. meta/tasks.jsonl   — v2.1 format: {"task_index": int, "task": str}
    """
    # ── Priority 1: tasks.parquet (v3.0, always present) ──────────────────────
    parquet_path = local_dir / TASKS_PARQUET_PATH
    if parquet_path.exists():
        import pyarrow.parquet as pq
        table = pq.read_table(parquet_path)
        df = table.to_pandas()
        # Index holds the task text; 'task_index' column holds the integer index.
        # The parquet may use __index_level_0__ as the actual index column.
        if df.index.name is None and "__index_level_0__" in df.columns:
            df = df.set_index("__index_level_0__")
        tasks = {int(row["task_index"]): str(idx) for idx, row in df.iterrows()}
    ...
    # No tasks file found, return empty
    return {}, {}
...
def load_episodes(local_dir: Path) -> dict:
    """Load episodes from episodes.jsonl (v2.1) or meta/episodes/*.parquet (v3.0)."""
    episodes_path = local_dir / EPISODES_PATH
    
    # Try v2.1 format first
    if episodes_path.exists():
        episodes = load_jsonlines(episodes_path)
        return {item["episode_index"]: item for item in sorted(episodes, key=lambda x: x["episode_index"])}
    
    # Try v3.0 format: meta/episodes/**/*.parquet (recursive glob)
    episodes_v3_dir = local_dir / "meta" / "episodes"
    if episodes_v3_dir.exists() and episodes_v3_dir.is_dir():
        import pyarrow.parquet as pq
        episodes = {}
        # Use rglob for recursive search
        for parquet_file in sorted(episodes_v3_dir.rglob("*.parquet")):
            table = pq.read_table(parquet_file)
            df = table.to_pandas()
            for _, row in df.iterrows():
                ep_idx = int(row["episode_index"])
                episodes[ep_idx] = row.to_dict()
        return episodes
    
    # Neither format found
    raise FileNotFoundError(f"Episodes file not found at {episodes_path} or {episodes_v3_dir}")
...
def load_episodes_stats(local_dir: Path) -> dict:
    """Load episode stats from episodes_stats.jsonl (v2.1) or return empty for v3.0."""
    episodes_stats_path = local_dir / EPISODES_STATS_PATH
    
    if not episodes_stats_path.exists():
        # v3.0 doesn't have this file, stats are embedded in episodes parquet or global stats
        return {}
