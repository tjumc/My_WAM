    ...
    def preprocess(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            Data: Dict[str, Any], lerobot sample in raw mcap obtained from dataset __getitem__:
                ...
                - "images": Dict[str, torch.Tensor] -> [image_obs_steps, C, H, W]
                - "action_is_pad": Optional, torch.Tensor -> [action_horizon,]
                - "state_is_pad": torch.Tensor -> [num_obs_steps,]
                - "image_is_pad": torch.Tensor -> [image_obs_steps,]
        ...
        Returns:
            Sample: Dict[str, Any], which can collated:
                ...
                - "pixel_values": torch.Tensor -> [num_input_cameras, image_obs_steps, C, H, W]
                - "image_is_pad": torch.Tensor -> [image_obs_steps,]
        ...
        for meta in self.shape_meta["images"]:
            ...
            image = data["images"][key]  # [image_obs_steps, C, H, W]
            assert image.ndim == 4, f"Expected 4 dimensions (image_obs_steps, C, H, W), got shape {image.shape}"
