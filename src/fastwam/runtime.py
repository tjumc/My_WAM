...
def build_datasets(data_cfg: DictConfig):
    ...
    train_ds = instantiate(_make_train_dataset_cfg(data_cfg, use_full_train=True))
    logger.info(
        "Validation dataset disabled; ignoring data.val_mode=%s and using full train dataset.",
        val_mode,
    )
...
def run_training(cfg: DictConfig):
    ...
    sys.stderr.write(f"[TRAIN rank={local_rank}] >>> run_training() entered\n")
    sys.stderr.flush()
    ...
    sys.stderr.write(f"[TRAIN rank={local_rank}] >>> instantiate(cfg.model) START — device={model_device}, dtype={model_dtype}\n")
    sys.stderr.flush()
    model = instantiate(cfg.model, model_dtype=model_dtype, device=model_device)
    sys.stderr.write(f"[TRAIN rank={local_rank}] >>> instantiate(cfg.model) DONE\n")
    sys.stderr.flush()

    sys.stderr.write(f"[TRAIN rank={local_rank}] >>> build_datasets() START\n")
    sys.stderr.write(f"[TRAIN rank={local_rank}]     dataset_dirs (train): {list(cfg.data.train.get('dataset_dirs', []))}\n")
    sys.stderr.flush()
    train_ds, val_ds = build_datasets(cfg.data)
    val_len = "None" if val_ds is None else len(val_ds)
    sys.stderr.write(f"[TRAIN rank={local_rank}] >>> build_datasets() DONE — train_len={len(train_ds)}, val_len={val_len}\n")
    sys.stderr.flush()

    sys.stderr.write(f"[TRAIN rank={local_rank}] >>> Wan22Trainer.train() START\n")
    sys.stderr.flush()
