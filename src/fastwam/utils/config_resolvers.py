...
def env_list(name: str) -> Any:
    """Read a newline-delimited list exported by a local launch script."""
    value = os.environ.get(name)
    if value is None:
        raise KeyError(f"Environment variable is not set: {name}")
    return OmegaConf.create([item for item in value.splitlines() if item])
