...
def main():
    ...
    # Event cues: gripper command edges + force derivative. Use robust z to make them commensurate.
    ...
    cue = smooth_1d(2.3*grip_imp + 0.18*force_imp, 5)
    score = robust_z(cp[:,None])[:,0] + cue
    score = smooth_1d(score, 5)
    # Candidate peaks, intentionally favor over-segmentation.
    peaks, props = find_peaks(score, distance=max(10,int(0.65*args.fps)), prominence=0.35)
    # Exclude boundary-unsafe edges.
    edge=int(1.5*args.fps)
    peaks=np.array([p for p in peaks if edge <= p < T-edge],dtype=int)
    # Always include gripper transitions as high-value anchors if not near an existing peak.
    grip_edges=np.flatnonzero(grip_imp>0.5)
    allcand=list(peaks)
    for g in grip_edges:
        if edge <= g < T-edge and all(abs(g-p)>int(0.30*args.fps) for p in allcand):
            allcand.append(int(g))
    # rank, NMS by 0.5s, then chronological output
    allcand=sorted(set(allcand), key=lambda i: float(score[i] + 0.8*grip_imp[i]), reverse=True)
    selected=[]
    for i in allcand:
    ...
    for eid,i in enumerate(selected):
        ...
        if grip_imp[i] > .5 and 'gripper_transition' not in signals:
            signals.insert(0,'gripper_transition')
        lerobot_frame=None
    ...
    report={
        ...
        'note':'candidate events are intentionally over-segmented; semantic verification/merging should be performed by a VLM or human.'
    }
