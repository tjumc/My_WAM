...
# Usage:
# bash versions/v3_4_2/run.sh HDF5 EPISODE_DIR TASK [LEROBOT_EPISODE] [LEROBOT_MAX_RAW_FRAME] [SCHEMA]
HDF5="$1"
EPISODE_DIR="$2"
TASK="$3"
LEROBOT_EPISODE="${4:-}"
...
OUT="$EPISODE_DIR/versions/v3_4_2"
PREV="$EPISODE_DIR/versions/v3_4_1"
FALLBACK="$EPISODE_DIR/versions/v3_4_0"
PROPOSAL_DIR="$OUT/proposal"
mkdir -p "$OUT"

echo "V3.4.2 task schema: $SCHEMA"
...
if [[ -d "$SOURCE/proposal" && "${V342_REPROPOSE:-0}" != "1" ]]; then
  rm -rf "$PROPOSAL_DIR"
...
if [[ -f "$SOURCE/entity_observations_base.jsonl" && "${V342_REOBSERVE:-0}" != "1" ]]; then
...
if [[ -f "$SOURCE/dense_cutlery_transition_evidence.jsonl" && "${V342_REDENSE:-0}" != "1" ]]; then
...
if [[ -n "${V342_GT:-}" ]]; then
  python "$EVENT_ROOT/evaluation/evaluate_temporal_annotations.py"     "$OUT/hierarchical_annotations.json" "$V342_GT"     --schema "$SCHEMA"     --output "$OUT/evaluation_temporal.json"
fi

python "$EVENT_ROOT/common/export_results.py"   --episode-dir "$EPISODE_DIR" --output-dir "$OUT"   --version "v3_4_2" --task "$TASK" --schema "$SCHEMA"   --hdf5 "$HDF5" --event-root "$EVENT_ROOT"

echo "V3.4.2 complete: $OUT"
echo "Ownership report: $OUT/hand_object_ownership.json"
