...
# Usage:
# GT=... KEEP_DEBUG=1 bash versions/v3_4_8/run.sh HDF5 EPISODE_DIR TASK [LEROBOT_EPISODE] [LEROBOT_MAX_RAW_FRAME] [SCHEMA]
HDF5="$1"
EPISODE_DIR="$2"
TASK="$3"
LEROBOT_EPISODE="${4:-}"
...
# Keep perception/proposals controlled relative to V3.4.7 whenever possible.
PROPOSAL_SRC=""
if PROPOSAL_SRC="$(source_artifact "$SOURCE" proposal 2>/dev/null)" && [[ "${V348_REPROPOSE:-0}" != "1" ]]; then
  rm -rf "$OUT/proposal"
...
BASE_SRC=""
if BASE_SRC="$(source_artifact "$SOURCE" entity_observations_base.jsonl 2>/dev/null)" && [[ "${V348_REOBSERVE:-0}" != "1" ]]; then
...
# V3.4.8 inherits V3.4.7 perception and adds candidate rehabilitation plus
...
if [[ -f "$OUT/targeted_observations.jsonl" && "${V348_RETARGET:-0}" != "1" ]]; then
  TARGET_ARGS+=(--reuse)
fi
python "$SCRIPT_DIR/../v3_4_5/targeted_reobserve.py" "${TARGET_ARGS[@]}"

echo "=== V3.4.8 reasoning pass 2 ==="
reasoning_pass
...
python "$EVENT_ROOT/common/export_results.py" \
  ...
  --version "v3_4_8" --task "$TASK" --schema "$SCHEMA" \
