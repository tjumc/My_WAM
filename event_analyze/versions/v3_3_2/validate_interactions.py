#!/usr/bin/env python3
"""V3.3.2 interaction-grounded validation for tracked skill candidates.
...
def affordance_temporal_rejections(skills, tracked, fps=30.0, tolerance_sec=1.0, skip_indices=None):
...
def initial_container_states(tracked):
    """Infer lifecycle state from the first reliable stable tracked segment."""
    stable = {
        "door": {"closed", "open"},
        "dish_rack": {"in", "out"},
        "cutlery_basket": {"in", "out"},
    }
    out = {}
    for entity, allowed in stable.items():
        state = "unknown"
        for seg in tracked.get("tracks", {}).get(entity, {}).get("segments", []):
            if (
                seg.get("state") in allowed
                and seg.get("support") == "observed"
                and float(seg.get("confidence", 0.0)) >= 0.55
            ):
                state = seg["state"]
                break
        out[entity] = state
...
def validate(args):
    ...
    final = []
    container_state = initial_container_states(tracked)
    ...
    for s in sorted(provisional, key=lambda x: (x["provisional_start_frame"], x["provisional_end_frame"])):
        ...
        if entity == "door":
            if state == "closed" and typ == "open_dishwasher_door":
            ...
            if state == "in" and typ.startswith("pull_out_"):
                ...
                final.append(s)
                container_state[entity] = "out" if typ.startswith("pull_out_") else "in"
                d["reasons"].append("lifecycle_initialized_from_first_valid_action")
    ...
    result = {
        ...
        "validation_policy": {
            ...
            "initial_container_state": initial_container_states(tracked),
