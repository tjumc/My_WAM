...
# Usage:
# GT=... KEEP_DEBUG=1 bash versions/v3_4_6/run.sh HDF5 EPISODE_DIR TASK [LEROBOT_EPISODE] [LEROBOT_MAX_RAW_FRAME] [SCHEMA]
HDF5="$1"
EPISODE_DIR="$2"
TASK="$3"
LEROBOT_EPISODE="${4:-}"
...
OUT="$EPISODE_DIR/versions/v3_4_6"
SOURCE="$EPISODE_DIR/versions/v3_4_5"
FALLBACK="$EPISODE_DIR/versions/v3_4_4"
mkdir -p "$OUT"

echo "V3.4.6 task schema: $SCHEMA"
...
# Keep perception/proposals controlled relative to V3.4.5 whenever possible.
PROPOSAL_SRC=""
if PROPOSAL_SRC="$(source_artifact "$SOURCE" proposal 2>/dev/null)" && [[ "${V346_REPROPOSE:-0}" != "1" ]]; then
  rm -rf "$OUT/proposal"
...
BASE_SRC=""
if BASE_SRC="$(source_artifact "$SOURCE" entity_observations_base.jsonl 2>/dev/null)" && [[ "${V346_REOBSERVE:-0}" != "1" ]]; then
...
if [[ "${V346_USE_LEGACY_DENSE:-0}" == "1" ]]; then
...
# V3.4.6 deliberately inherits V3.4.5 local perception/tracking/validation so
# the controlled experiment isolates conservative assimilation + task decoding.
reasoning_pass () {
...
echo "=== V3.4.6 reasoning pass 1 ==="
reasoning_pass

python "$SCRIPT_DIR/conservative_reasoning.py" \
  --output-dir "$OUT" --mode capture

python "$SCRIPT_DIR/../v3_4_5/detect_ambiguities.py" \
  --output-dir "$OUT" --schema "$SCHEMA" --hdf5 "$HDF5"

# Reuse V3.4.5 targeted observations only when every request signature matches.
# Otherwise issue fresh targeted calls. This preserves controlled comparisons.
if [[ "${V346_RETARGET:-0}" != "1" ]]; then
  rm -f "$OUT/targeted_observations.jsonl"
  if python "$SCRIPT_DIR/reuse_targeted.py" --output-dir "$OUT" --source-dir "$SOURCE"; then
    echo "Exact V3.4.5 targeted evidence reused."
  else
    rm -f "$OUT/targeted_observations.jsonl"
  fi
fi
...
if [[ -f "$OUT/targeted_observations.jsonl" && "${V346_RETARGET:-0}" != "1" ]]; then
  TARGET_ARGS+=(--reuse)
fi
python "$SCRIPT_DIR/../v3_4_5/targeted_reobserve.py" "${TARGET_ARGS[@]}"

echo "=== V3.4.6 reasoning pass 2 ==="
reasoning_pass
...
python "$EVENT_ROOT/common/export_results.py" \
  ...
  --version "v3_4_6" --task "$TASK" --schema "$SCHEMA" \
