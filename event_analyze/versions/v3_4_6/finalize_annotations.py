#!/usr/bin/env python3
"""V3.4.6 dependency-aware final task-graph decoder.

Unlike V3.4.5, the task-completion frontier is not defined by the last predicted
...
def apply_prerequisite_state_inference(schema, phase, state, report):
    """Infer latent prerequisite state from an already accepted dependent use.

    This never invents a missing skill. It only updates hidden lifecycle state:
    if a reliable downstream action could occur only while a prerequisite entity
    was in a declared state, that prerequisite state is treated as holding at
    that time.
    """
    for used in phase_entity_uses(phase):
        ...
        for req in cfg.get("requires", []) or []:
            ...
            if not parent or required is None:
                continue
            current = state.get(parent, "unknown")
            if current == required:
                continue
            state[parent] = required
            row = {
                "supporting_skill": phase.get("skill_type"),
                "supporting_entity": used,
                "prerequisite_entity": parent,
            ...
            phase.setdefault("latent_state_inference", []).append(row)
            report.setdefault("latent_prerequisite_state_inferences", []).append(row)


def lifecycle_filter(phases, transitions, initial, report, schema):
    state = dict(initial)
    kept = []
    for p in sorted(phases, key=lambda x: (x["raw_start_frame"], x["raw_end_frame"])):
        apply_prerequisite_state_inference(schema, p, state, report)
...
def terminal_closure_info(schema, phases, initial, transitions):
    ...
    for entity, cfg in schema.get("entities", {}).items():
        ...
        lifecycle = []
        for p in sorted(phases, key=lambda x: (x["raw_start_frame"], x["raw_end_frame"])):
            # A dependent accepted phase may prove that this prerequisite entity
            # had already reached the required latent state even if the explicit
            # transition skill was missed.
            for used in phase_entity_uses(p):
                for req in schema.get("entities", {}).get(used, {}).get("requires", []) or []:
                    if req.get("entity") == entity and req.get("state") is not None:
                        cur = req.get("state")
...
def main():
    ...
    ann.setdefault("notes", []).append(
        "V3.4.6 derives task completion from schema dependencies and expected final states; final predictions cannot extend their own validity horizon."
    )
