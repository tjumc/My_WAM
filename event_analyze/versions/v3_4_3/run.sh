...
# Usage:
# bash versions/v3_4_3/run.sh HDF5 EPISODE_DIR TASK [LEROBOT_EPISODE] [LEROBOT_MAX_RAW_FRAME] [SCHEMA]
HDF5="$1"
EPISODE_DIR="$2"
TASK="$3"
LEROBOT_EPISODE="${4:-}"
...
OUT="$EPISODE_DIR/versions/v3_4_3"
PREV="$EPISODE_DIR/versions/v3_4_2"
FALLBACK="$EPISODE_DIR/versions/v3_4_1"
PROPOSAL_DIR="$OUT/proposal"
mkdir -p "$OUT"

echo "V3.4.3 task schema: $SCHEMA"
...
if [[ -d "$SOURCE/proposal" && "${V343_REPROPOSE:-0}" != "1" ]]; then
  rm -rf "$PROPOSAL_DIR"
...
if [[ -f "$SOURCE/entity_observations_base.jsonl" && "${V343_REOBSERVE:-0}" != "1" ]]; then
...
if [[ -f "$SOURCE/dense_cutlery_transition_evidence.jsonl" && "${V343_REDENSE:-0}" != "1" ]]; then
