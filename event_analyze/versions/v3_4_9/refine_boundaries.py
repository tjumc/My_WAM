#!/usr/bin/env python3
"""V3.4.9 signal-grounded refinement with pass1-anchor protection."""
import argparse
import json
...
def mark_pass1_anchors(skills, anchors):
    """Greedily match reconciled phases to validated pass1 semantic anchors."""
    pairs = []
    for i, skill in enumerate(skills):
        key = (skill.get("skill_type"), skill.get("fine_entity") or skill.get("entity"))
        ss, se = span(skill)
        for j, anchor in enumerate(anchors):
            akey = (anchor.get("skill_type"), anchor.get("fine_entity") or anchor.get("entity"))
            if key != akey:
                continue
            a0, a1 = span(anchor)
            overlap = max(0, min(se, a1) - max(ss, a0) + 1)
            gap = abs((ss + se) / 2.0 - (a0 + a1) / 2.0)
            if overlap or gap <= 45:
                pairs.append((gap, -overlap, i, j, anchor))
    pairs.sort(key=lambda x: (x[0], x[1]))
    matched_skills, matched_anchors = set(), set()
    for _, _, i, j, anchor in pairs:
        if i in matched_skills or j in matched_anchors:
            continue
        matched_skills.add(i)
        matched_anchors.add(j)
        skills[i]["boundary_anchor"] = {
            "source": "validated_pass1_skill",
            "pass1_skill_id": anchor.get("skill_id"),
            "pass1_span": list(span(anchor)),
        }
...
def main():
    ...
    anchor_doc = json.load(open(out / "diagnostics/pass1_anchors.json", encoding="utf-8")) \
        if (out / "diagnostics/pass1_anchors.json").exists() else {"skill_sequence": []}
    mark_pass1_anchors(skills, anchor_doc.get("skill_sequence", []))
    ...
    #
    # The upstream provisional interval remains a semantic ownership
    ...
    def resolve_pair(a, b):
        ...
        a_anchor = bool(a.get("boundary_anchor"))
        b_anchor = bool(b.get("boundary_anchor"))
        ...
        # First decide whether both duration floors fit. The pass1 anchor's
        # complete provisional span is a preference, not a hard constraint:
        # otherwise a long anchor can consume a valid new phase entirely.
        if feasible_lo <= feasible_hi:
            floor_lo, floor_hi = feasible_lo, feasible_hi
            if a_anchor and not b_anchor:
                feasible_lo = max(feasible_lo, a_pe)
            elif b_anchor and not a_anchor:
                feasible_hi = min(feasible_hi, b_ps - 1)
            if feasible_lo > feasible_hi:
                feasible_lo, feasible_hi = floor_lo, floor_hi
                mode += "_anchor_floor_fallback"
            ...
            # When both duration floors cannot fit, give the validated anchor
            # the available interval and let the new candidate fail closed.
            if a_anchor != b_anchor:
                if a_anchor:
                    cut = min(old_b[1], max(old_a[0] + need_a - 1, a_pe))
                    mode += "_preserve_left_anchor"
                else:
                    cut = min(old_b[1] - need_b, b_ps - 1)
                    cut = max(old_a[0] - 1, cut)
                    mode += "_preserve_right_anchor"
            else:
                # Two anchors (or two new phases) still use the closest
                # semantic-order cut and expose any infeasible duration.
        ...
        b["raw_start_frame"] = (
            int(cut) + 1
            if a_anchor and not b_anchor and int(cut) >= old_b[1]
            else min(old_b[1], int(cut) + 1)
        )
        ...
        info = {
            ...
            "pair_duration_conflict": duration_conflict,
            ...
            "protected_anchor_role": (
                "left" if a_anchor and not b_anchor else
                "right" if b_anchor and not a_anchor else None
            ),
        }
        left_info = {**info, "role": "left"}
        right_info = {**info, "role": "right"}
        if a_anchor and not b_anchor:
            left_info["duration_conflict"] = False
        elif b_anchor and not a_anchor:
            right_info["duration_conflict"] = False
        a.setdefault("boundary_arbitration", []).append(left_info)
        b.setdefault("boundary_arbitration", []).append(right_info)
    ...
    result = {
        ...
        "notes": [
            "Phases use interaction-validated trajectory-level entity-state-machine skills, not LLM-generated phases.",
            ...
            "Validated pass1 anchors retain their provisional span and minimum duration when overlapping with a later candidate whenever feasible.",
        ],
    }
