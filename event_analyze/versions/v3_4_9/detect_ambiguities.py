#!/usr/bin/env python3
"""V3.4.9 ambiguity-triggered targeted re-observation.
...
def post_use_robot_event(out, cfg, start, end):
    """Locate an interaction proposal after an accepted receptacle use."""
    proposal = load_json(out / "proposal/candidate_events.json", {})
    arm = (cfg.get("robot_interaction") or {}).get("gripper", "right")
    rows = [
        event for event in proposal.get("candidate_events", [])
        if start <= int(event.get("raw_frame", -1)) <= end
        and any(str(signal).startswith(arm + "_") for signal in event.get("signals", []))
    ]
    return max(rows, key=lambda x: float(x.get("score", 0.0)), default=None)
...
def main():
    ...
    for s in sorted(
        ...
        entity = s.get("entity")
        target = (s.get("state_transition") or {}).get("after")
        usage = schema.get("entities", {}).get(target, {}).get("usage_state")
        if target in tmap and usage is not None and entity != target:
    ...
    for entity, cfg in schema.get("entities", {}).items():
        ...
        last_placement = max((
            int(s.get("provisional_end_frame", 0)) for s in accepted_skills
            if s.get("entity") != entity
            and (s.get("state_transition") or {}).get("after") == entity
        ), default=None)
        event = (post_use_robot_event(out, cfg, last_placement + 1, hi)
                 if last_placement is not None and last_placement < hi else None)
        # A query after the last confirmed placement is more informative than
        # a stale rejected transition before that placement. Its robot event
        # anchors the search, while direct vision and validation still decide
        # whether any phase exists.
        if event is not None and (cand is None or int(cand.get("provisional_start_frame", 0)) <= last_placement):
            width = max(2, int(round(float(policy.get("window_sec", 4.5)) * args.fps)))
            s = max(0, last_placement + 1)
            e = min(T - 1, max(s + width - 1, int(event["raw_frame"]) + pad))
            support = float(event.get("score", 0.0))
            source_kind = "post_placement_robot_event"
        elif cand is not None:
            s, e = padded_span(cand, pad, T)
            source_kind = "existing_candidate"
        else:
            s, e = fallback_window(anchor + 1, False, policy, args.fps, T)
            support = 0.0
            source_kind = "fallback_window"
        ...
        add_request(
            ...
            source={
                ...
                "robot_event": event if source_kind == "post_placement_robot_event" else None,
                "last_accepted_placement_end": last_placement,
                "fps": args.fps,
            },
