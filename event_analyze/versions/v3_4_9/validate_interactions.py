#!/usr/bin/env python3
"""V3.4.9 interaction-grounded validation with latent lifecycle implications.
...
def gripper_state_support(signals, start, end, cfg):
    """Schema-driven gripper-state evidence for contact manipulation.

    This is deliberately a bridge, not a standalone action detector. It may
    replace missing visual hand-contact only when a direction-consistent visual
    state transition and robot interaction signal also support the candidate.
    """
    if not cfg:
        return {"enabled": False, "matched": False}
    expected = cfg.get("expected_gripper_state")
    if expected not in {"open", "closed"}:
        return {"enabled": False, "matched": False}

    arr = signals.get("gripper_command")
    if arr is None or len(arr) == 0:
        return {"enabled": True, "matched": False, "reason": "missing_gripper_command"}
    ...
    threshold = float(cfg.get("open_threshold", 50.0))
    vals = arr[s:e]
    if expected == "open":
        mask = vals > threshold
    else:
    ...
    required = float(cfg.get("min_state_fraction", 0.65))
    return {
        "enabled": True,
        ...
        "matched": bool(fraction >= required),
        "window": [s, e - 1],
    }
...
def container_evidence(skill, obs, dense_rows=None):
    ...
    direction_only = [o for o in nearby if direction_match(o, entity, skill["skill_type"])]
    directed = [o for o in direction_only if hand_contact(o, tokens)]
    ...
    return {
        ...
        "direction_only_event_ids": [int(o["_event_id"]) for o in direction_only],
        "contact_count": len(contacts) + len(dense_contact),
        "directed_count": len(directed) + len(dense_contact),
        "direction_only_count": len(direction_only) + len(dense_contact),
...
def phase_state_implications(phase, schema):
    """Return schema-required hidden states supported by an accepted phase."""
    entities = schema.get("entities", {})
    used = {
        phase.get("entity"),
        phase.get("fine_entity"),
        (phase.get("state_transition") or {}).get("after"),
    }
    rows = []
    for used_entity in used:
        cfg = entities.get(used_entity, {})
        for req in cfg.get("requires", []) or []:
            if req.get("entity") and req.get("state") is not None:
                rows.append({
                    "entity": req["entity"],
                    "state": req["state"],
                    "reason": "accepted_use_requires_prerequisite_state",
                    "supporting_entity": used_entity,
                })

    target = (phase.get("state_transition") or {}).get("after")
    usage = entities.get(target, {}).get("usage_state")
    if target and usage is not None:
        rows.append({
            "entity": target,
            "state": usage,
    ...
    unique = {}
    for row in rows:
        unique[(row["entity"], row["state"], row["reason"])] = row
    return list(unique.values())


def apply_validation_implications(phase, schema, container_state, decision):
    applied = []
    for implication in phase_state_implications(phase, schema):
        entity, required = implication["entity"], implication["state"]
        previous = container_state.get(entity, "unknown")
        if previous == required:
            continue
        container_state[entity] = required
        applied.append({
            **implication,
            ...
            "supporting_span": [
                phase.get("provisional_start_frame"),
                phase.get("provisional_end_frame"),
            ],
        })
    if applied:
        decision.setdefault("latent_state_implications", []).extend(applied)
    return applied
...
def validate(args):
    ...
    global OBJECT_CONFIG, SKILL_LABELS, INTERACTION_SIGNATURE
    ...
    for entity, cfg in cr.items():
        ...
        INTERACTION_SIGNATURE[entity] = dict(cfg.get("robot_interaction") or {})
    ...
    for i, s0 in enumerate(skills):
        ...
        if s.get("entity") in CONTAINER_ACTIONS:
            ...
            bridge_cfg = INTERACTION_SIGNATURE.get(entity, {})
            gripper_ev = gripper_state_support(
                signals,
                s["provisional_start_frame"],
                s["provisional_end_frame"],
                bridge_cfg,
            )
            d["gripper_signature"] = gripper_ev
            bridge_min_signal = float(
                bridge_cfg.get("min_signal_score", max(args.min_signal_score, 0.12))
            )
            visual_direction_gripper_bridge = (
                ev["contact_count"] == 0
                and bool(bridge_cfg.get("allow_contact_bridge", False))
                and int(ev.get("direction_only_count", 0)) > 0
                and bool(gripper_ev.get("matched"))
                and float(sig["score"]) >= bridge_min_signal
            )

            if ev["contact_count"] == 0 and not visual_direction_gripper_bridge:
                d["reasons"].append("no_entity_specific_hand_contact")
                if int(ev.get("direction_only_count", 0)) > 0:
                    d["reasons"].append("direction_seen_but_contact_bridge_not_supported")
                decisions.append(d)
                continue

            if visual_direction_gripper_bridge:
                d["accepted"] = True
                d["reasons"].append(
                    "schema_gripper_plus_directional_visual_contact_bridge"
                )
                d["reasons"].append("robot_signal_support")
                provisional.append(s)
                decisions.append(d)
                continue
    ...
    for s in sorted(provisional, key=lambda x: (x["provisional_start_frame"], x["provisional_end_frame"])):
        ...
        if entity not in container_state:
            final.append(s)
            apply_validation_implications(s, schema, container_state, d)
            continue
    ...
    for new_id, s in enumerate(final):
        ...
        s["validation"] = {
            ...
            "latent_state_implications": d.get("latent_state_implications", []),
        }

    result = {
        "annotation_version": "v3.4.9",
        ...
        "validation_policy": {
            ...
            "schema_driven_gripper_contact_bridge": True,
            "accepted_placement_implications_in_lifecycle_validation": True,
            "candidate_rehabilitation_requires_direct_targeted_direction_and_robot_support": True,
        },
    }
