#!/usr/bin/env python3
"""Revalidate rejected transitions and discover robot-backed lifecycle gaps."""
import argparse
import json
...
def prepare(out, schema):
    ...
    audit = {
        "annotation_version": "v3.4.9",
        "policy": "revalidate rejected or post-placement robot-event transitions only with a selected matching query, direct directional targeted evidence, and a placement-implied lifecycle gap",
        "candidates": [],
    }
    ...
    # Pass1 cannot preserve a phase it never proposed. A post-placement robot
    # event may seed one new hypothesis, but only if the selected targeted
    # observation directly shows the schema transition direction. The normal
    # interaction validator still decides whether the candidate is accepted.
    for req in requests:
        source = req.get("source") or {}
        event = source.get("robot_event")
        if req.get("kind") != "unfinished_lifecycle" or source.get("source_kind") != "post_placement_robot_event" or not event:
            continue
        ...
        usage = cfg.get("usage_state")
        skills = [
            x["skill_type"] for x in cfg.get("transitions", [])
            if x.get("from") == usage and x.get("to") == final
        ]
        if len(skills) != 1:
            continue
        skill_type = skills[0]
        support = float(event.get("score", 0.0))
        center_frame = int(event["raw_frame"])
        pad = max(1, int(round(float(schema.get("targeted_perception", {}).get("padding_sec", 1.25)) * float(source.get("fps", 30.0)))))
        candidate_start = max(int(req["start_frame"]), center_frame - pad)
        candidate_end = min(int(req["end_frame"]), center_frame + pad)
        labels = schema.get("skill_labels", {}).get(skill_type, {})
        candidate = {
            "skill_id": -1,
            "skill_type": skill_type,
            "instruction_zh": labels.get("zh", skill_type),
            "instruction_en": labels.get("en", skill_type.replace("_", " ")),
            "entity": entity,
            "active_arm": (cfg.get("robot_interaction") or {}).get("gripper", "right"),
            "provisional_start_frame": candidate_start,
            "provisional_end_frame": candidate_end,
            "evidence_event_ids": [int(event["event_id"]), 20000 + int(req["request_id"])],
            "confidence": 0.0,
            "state_transition": {"before": usage, "after": final},
            "inference_mode": "robot_event_with_targeted_direction",
            "source": "post_placement_lifecycle_search",
        }
        record = {
            "entity": entity, "skill_type": skill_type,
            "candidate_span": list(span(candidate)), "robot_support": support,
            "robot_event_id": event.get("event_id"), "status": "not_rehabilitated",
        }
        if support < min_robot:
            record["reason"] = "weak_robot_proposal"
            audit["candidates"].append(record)
            continue
        evidence = targeted_transition_evidence(targeted, req, candidate, schema, min_conf)
        if evidence is None:
            record["reason"] = "no_high_confidence_directional_targeted_observation"
            audit["candidates"].append(record)
            continue
        ...
        if gap is None:
            ...
            audit["candidates"].append(record)
            continue
        candidate["confidence"] = round(min(evidence["confidence_before_after"]), 3)
        rehab_id = f"robot_event:{event['event_id']}:{entity}:{skill_type}"
        candidate["candidate_rehabilitation"] = {
            "rehabilitation_id": rehab_id,
            "state_implication": gap,
            "targeted_evidence": evidence,
        ...
        if existing:
            ...
            candidates.append(candidate)
            record["queued_source"] = "post_placement_robot_event"
        record.update({
            "status": "queued_for_revalidation",
            "reason": "robot_event_targeted_direction_and_state_implication_agree",
            "targeted_evidence": evidence,
            "state_implication": gap,
            "rehabilitation_id": rehab_id,
        })
