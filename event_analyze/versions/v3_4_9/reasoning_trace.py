#!/usr/bin/env python3
"""Build compact V3.4.9 pass1 -> raw pass2 -> reconciled -> final trace."""
import argparse
import difflib
import json
...
def main():
    ...
    rehabilitation = load_json(diag / "candidate_rehabilitation.json", {"candidates": []})
    ...
    trace = {
        ...
        "candidate_rehabilitation": {
            "num_considered": len(rehabilitation.get("candidates", [])),
            "num_queued": sum(
                bool(x.get("rehabilitation_id"))
                for x in rehabilitation.get("candidates", [])
            ),
            "num_accepted_by_validator": sum(
                x.get("status") == "accepted_by_interaction_validator"
                for x in rehabilitation.get("candidates", [])
            ),
            "candidates": rehabilitation.get("candidates", []),
        },
        "reconciled": {
