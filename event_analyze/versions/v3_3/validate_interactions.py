#!/usr/bin/env python3
"""V3.3 interaction-grounded validation for tracked skill candidates.
...
def container_evidence(skill, obs):
    ...
    contacts = [o for o in nearby if hand_contact(o, token)]
...
def mark_short_reverse_pairs(skills, max_gap_frames=30):
    rejected = set()
    by_entity = {}
...
def validate(args):
    ...
    short_reverse = mark_short_reverse_pairs(skills, int(round(args.short_reverse_sec * args.fps)))
    decisions = []
    provisional = []
