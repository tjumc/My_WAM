#!/usr/bin/env python3
"""V3.3 trajectory-level entity state tracking.
...
def build_anchor_evidence(obs):
    ...
    for o in obs:
        ...
        dense_cutlery = bool(o.get("_v33_dense_cutlery"))
        for side, frame_key in (("before", "_window_start_frame"), ("after", "_window_end_frame")):
            ...
            for entity in SPECS:
                ...
                c = float(cf.get(entity, 0.0)) * q
                # Dense relative-motion evidence directly compares the small basket
                # against the dish rack across many time points, so it is more
                # informative than a generic single-window absolute-state guess.
                if dense_cutlery and entity == "cutlery_basket":
                    c *= 1.8
...
def emission_score(spec, state, evidence):
    ...
    for e in evidence:
        ...
        if kind == "state":
            ...
            score += 3.5 * c if state == v else -2.5 * c
            if state == v:
                direct_support = max(direct_support, c)
...
def viterbi_track(entity, spec, frames, by_frame):
    ...
    for i, (f, st) in enumerate(zip(frames, path)):
        ...
        match = [e for e in ev if e["kind"] in ("state", "must") and e["value"] == st]
        direct = max((float(e["confidence"]) for e in match), default=0.0)
        src = sorted({int(e["event_id"]) for e in ev if e["kind"] in ("state", "must") and e["value"] == st})
...
def main():
    ...
    by_frame = build_anchor_evidence(obs)
    ...
    result = {
        ...
        "notes": [
            ...
            "Dense relative-motion evidence receives higher weight only for its targeted entity.",
