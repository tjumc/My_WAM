#!/usr/bin/env python3
"""V3.3.1 dense targeted temporal observation for small/occluded articulated entities.
...
def pseudo_observation(x, synthetic_id):
    start_state = x["start_state"]
    end_state = x["end_state"]
    motion = x["relative_motion"]
    conf = float(x["confidence"])

    if motion == "moving_out":
        if start_state in {"uncertain", "not_visible"}:
            start_state = "in"
        if end_state in {"uncertain", "not_visible"}:
            end_state = "out"
    elif motion == "moving_in":
        if start_state in {"uncertain", "not_visible"}:
            start_state = "out"
        if end_state in {"uncertain", "not_visible"}:
            end_state = "in"

    before = {k: "uncertain" for k in STATE_KEYS}
    after = {k: "uncertain" for k in STATE_KEYS}
    cb = {k: 0.0 for k in STATE_KEYS}
    ca = {k: 0.0 for k in STATE_KEYS}

    before["cutlery_basket"] = start_state
    after["cutlery_basket"] = end_state
    cb["cutlery_basket"] = conf
    ca["cutlery_basket"] = conf

    relation = x.get("right_hand_relation", "uncertain")
    if relation in {"contact_cutlery_basket", "pulling_cutlery_basket", "pushing_cutlery_basket"}:
        before["right_hand"] = "contact_cutlery_basket"
        after["right_hand"] = "contact_cutlery_basket"
        cb["right_hand"] = conf
        ca["right_hand"] = conf
    ...
    return {
        ...
        "confidence_after": ca,
        "visible_entities": ["cutlery_basket"] if conf >= 0.6 else [],
        "evidence": x.get("evidence", []),
        "window_quality": {"good": "good", "partial": "partial", "poor": "poor"}.get(x.get("visibility"), "partial"),
        "_event_id": int(synthetic_id),
        "_raw_frame": int(x["_raw_frame"]),
        "_time_sec": None,
        "_window_start_frame": int(x["_window_start_frame"]),
        "_window_end_frame": int(x["_window_end_frame"]),
        "_detector_score": None,
        "_detector_signals": ["dense_targeted_relative_motion"],
        "_sensor_context": {},
        "_v33_dense_cutlery": x,
        "_v331_dense_target_entity": "cutlery_basket",
    }
...
def main():
    ...
    for w in windows:
        ...
        # V3.3.1 only injects directional motion. Stationary dense windows were
        # useful diagnostically but created contradictory in/out anchors when
        # overlapped with a true transition.
        if obj["confidence"] >= args.min_merge_confidence and directional:
            pseudos.append(pseudo_observation(obj, 10000 + int(w["window_id"])))
    ...
    augmented = list(base) + pseudos
    augmented.sort(key=lambda x: (int(x["_raw_frame"]), int(x["_event_id"])))
    (out / "entity_observations.jsonl").write_text(
        "\n".join(json.dumps(x, ensure_ascii=False) for x in augmented) + "\n",
        encoding="utf-8",
    )
    print(f"dense accepted observations: {len(pseudos)}/{len(dense)}")
