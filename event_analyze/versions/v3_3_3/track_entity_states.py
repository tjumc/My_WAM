#!/usr/bin/env python3
"""V3.3.3 trajectory-level entity state tracking.
...
def inject_generic_motion_source_priors(by_frame, generic_frames, obs):
    """Use direct motion observations to constrain the immediately preceding state.

    Example: an early 'opening' observation is evidence that the door was
    closed immediately before it. This prevents later Viterbi smoothing from
    back-filling 'open' all the way to the trajectory start.
    """
    motion_source = {
        "door": {"opening": "closed", "closing": "open"},
        "dish_rack": {"moving_out": "in", "moving_in": "out"},
        "cutlery_basket": {"moving_out": "in", "moving_in": "out"},
    }
    ...
    for o in obs:
        ...
        if prev is None:
            continue
        for entity, mapping in motion_source.items():
            ...
            by_frame[prev][entity].append({
                ...
                "source": "generic_motion_source_prior",
            })
...
def inject_dense_motion_evidence(by_frame, targeted_frames, dense_rows):
    ...
    for x in dense_rows:
        ...
        endpoint = "out" if motion == "moving_out" else "in"
