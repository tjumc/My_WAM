#!/usr/bin/env python3
"""V3.3.3 interaction-grounded validation for tracked skill candidates.
...
def initial_container_states(obs, max_observations=5, min_conf=0.70):
    ...
    rules = {
        "door": {
            "stable": {"closed", "open"},
            "motion_source": {"opening": "closed", "closing": "open"},
        },
        "dish_rack": {
            "stable": {"in", "out"},
...
def validate(args):
    ...
    for s in sorted(provisional, key=lambda x: (x["provisional_start_frame"], x["provisional_end_frame"])):
        ...
        if entity in {"dish_rack", "cutlery_basket"} and container_state.get("door") != "open":
            d["accepted"] = False
            d["reasons"].append("container_interaction_before_door_open")
            continue
    ...
    result = {
        ...
        "validation_policy": {
            ...
            "container_requires_open_door": True,
            "door_span_preserved_from_tracker": True,
        },
    }
