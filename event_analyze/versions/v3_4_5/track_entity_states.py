#!/usr/bin/env python3
"""V3.4.5 trajectory-level entity state tracking.
...
def build_anchor_evidence(obs):
    ...
    Generic observations define the shared baseline timeline. Any targeted
    observation adds frames only to the entities it explicitly re-observed, so
    focused perception cannot perturb unrelated Viterbi paths merely by adding
    empty/uncertain timestamps.
    """
    ...
    for o in obs:
        ...
        v344_targets = set()
        if o.get("_v344_targeted"):
            logical_targets = set(o.get("_target_entities") or [])
            for key, spec in SPECS.items():
                if spec.get("schema_entity") in logical_targets:
                    v344_targets.add(key)
        ...
        if dense_target:
            ...
            for key in v344_targets:
                targeted_frames[key].update(obs_frames)
        else:
        ...
        for side, frame_key in (("before", "_window_start_frame"), ("after", "_window_end_frame")):
            ...
            for entity in SPECS:
                if v344_targets and entity not in v344_targets:
                    continue

                v = st.get(entity, "uncertain")
                conf = max(0.0, min(1.0, float(cf.get(entity, 0.0)) * q))
                weight = 1.0
                source = "generic"
                if dense_target == entity:
                    weight = 1.8
                    source = "dense_targeted"
                elif entity in v344_targets:
                    weight = 1.35
                    source = "ambiguity_targeted"
                ...
                if entity.endswith("_location") and not dense_target:
                    for kind, hand, hc in hand_identity_evidence(o, side, entity):
                        by_frame[frame][entity].append({
                            ...
                            "source_weight": 1.15 if entity in v344_targets else 1.0,
                            "event_id": int(o["_event_id"]),
                            "side": side,
                            "source": source,
                        })
...
def main():
    ...
    result = {
        ...
        "notes": [
            ...
            "Targeted frames are added only to explicitly re-observed entity timelines; unrelated tracks keep the generic baseline timeline.",
            ...
            "Focused ambiguity-triggered observations use a separate source weight while confidence remains clamped to [0,1].",
