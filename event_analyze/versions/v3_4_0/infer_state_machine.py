#!/usr/bin/env python3
"""V3.4.0 skill inference from trajectory-level tracked entity states.
...
def object_placement_skills(track, entity, fps=30.0, retry_sec=3.0, stable_target_sec=1.2):
    """Build object-centric manipulation episodes before emitting skills.

    One episode starts when an object leaves a stable source state and enters a
    hand state. Short target contacts followed by a re-grasp are treated as an
    unfinished placement/retry rather than a second semantic skill.
    """
    segs = track.get("segments", [])
    target, skill_type, obj_name = OBJECT_RULES[entity]
    hand_states = {"right_hand", "left_hand"}
    retry_gap = int(round(float(retry_sec) * float(fps)))
    stable_target = int(round(float(stable_target_sec) * float(fps)))
    out = []
    ...
    while i < len(segs):
        ...
        episode_start_i = i
        first_hand = segs[i]
        hand_conf = float(first_hand.get("confidence", 0.0))
        event_end_i = i
        final_target_i = None
        retried = False
        k = i + 1
        ...
        while k < len(segs):
            ...
            if st == target:
                dwell = int(segs[k]["end_frame"]) - int(segs[k]["start_frame"]) + 1
                next_hand = next(
                    (h for h in range(k + 1, len(segs))
                     if segs[h]["state"] in hand_states
                     and int(segs[h]["start_frame"]) - int(segs[k]["end_frame"]) <= retry_gap),
                    None,
                )
                # A short target contact followed by a quick re-grasp is not a
                # completed semantic placement yet.
                if dwell < stable_target and next_hand is not None:
                    retried = True
                    event_end_i = next_hand
                    k = next_hand + 1
                    continue
                final_target_i = k
                event_end_i = k
            ...
            if st in hand_states:
                event_end_i = k
                k += 1
                continue

            # Returning briefly to tabletop/other can be a failed grasp.
            next_hand = next(
                (h for h in range(k + 1, len(segs))
                 if segs[h]["state"] in hand_states
                 and int(segs[h]["start_frame"]) - int(segs[k]["end_frame"]) <= retry_gap),
        ...
        if inferred:
            conf *= 0.78
        mode = "object_centric_episode_with_affordance_completion" if inferred else "object_centric_episode"

        add_skill(
            out, skill_type, obj_name,
            first_hand["start_frame"], target_seg["start_frame"],
            segment_events(segs, prev_table, final_target_i),
            conf,
            {
                "before": "tabletop",
                "held": first_hand["state"],
                "after": target,
        ...
        if retried:
            ...
            out[-1]["retry_merge_window_sec"] = float(retry_sec)
...
def main():
    ...
    for logical_name, cfg in pr.items():
        key = cfg["observation_key"]
        OBJECT_RULES[key] = (cfg["target"], cfg["skill_type"], logical_name)
    ...
    for entity in OBJECT_RULES:
        if entity in tracks:
            skills.extend(object_placement_skills(
                tracks[entity], entity, args.fps, args.retry_merge_sec, args.stable_target_sec
            ))
    ...
    result = {
        ...
        "notes": [
            ...
            "Object-centric manipulation episodes are formed before skill emission; brief target contacts and re-grasps remain inside one semantic placement skill.",
